...
class DemoAdapter(private val list:List<DemoModel>):RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return  if (viewType == DemoModel.SECTION_HEADER){
            SectionHeaderVH(LayoutInflater.from(parent.context).inflate(R.layout.item_section_header,parent,false))
        }else{
            SectionContentVH(LayoutInflater.from(parent.context).inflate(R.layout.item_section_content,parent,false))
        }
    }
